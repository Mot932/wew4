...
def start_fight(hero: list) -> None:
    ...
    [10] inventory - список предметов
    """
    enemy = make_hero(hp_now=50, xp_now=30, money=50, inventory=["меч Гладиатора", "шапка Мономаха", "кольчуга гладиатора"])
...
def choose_option(hero: list, text: str, options: list) -> int:
    ...
    try: # что пробуем сделать?
        option = int(option)
    except: # сработает если try с ошибкой
...
def visit_hub(hero: list) -> None:
    ...
    input("\nНанжите ENTER чтобы продолжить")
